...
int main(){
    ...
    for(int i=0; i<n; i++) for(int j=0; j<n; j++) B[i][j]=false; //Oznaczenie, wszystkich elementów grafu jako false
    ...
    while(!z.empty()){ //Dopoki kolejka z nie jest pusta
        ...
        B[w.x][w.y]=true; //Punkt w zostaje oznaczony jako true
        ...
        if(w.x<n-1) { v.x=w.x+1; v.y=w.y; if ((A[v.x][v.y]=='.' || A[v.x][v.y]=='6') && B[v.x][v.y]==false){ z.push(v); P[v.x][v.y]=w; } } 
        if(w.x>0)   { v.x=w.x-1; v.y=w.y; if ((A[v.x][v.y]=='.' || A[v.x][v.y]=='6') && B[v.x][v.y]==false){ z.push(v); P[v.x][v.y]=w; } }
        if(w.y<n-1) { v.x=w.x; v.y=w.y+1; if ((A[v.x][v.y]=='.' || A[v.x][v.y]=='6') && B[v.x][v.y]==false){ z.push(v); P[v.x][v.y]=w; } }
        if(w.y>0)   { v.x=w.x; v.y=w.y-1; if ((A[v.x][v.y]=='.' || A[v.x][v.y]=='6') && B[v.x][v.y]==false){ z.push(v); P[v.x][v.y]=w; } }
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<n; j++) {if(B[i][j]==true) cout<<(char)219; else cout<<'X';} //Wypisuje wszystkie punkty, gdzie B jest równe true
        cout<<endl;
    }
